...
import { gerarCertificadoPdfService } from "../services/certificadoService";

dotenv.config();

const QUEUE_NAME = process.env.QUEUE_CERTIFICADO_NAME as string;
    ...
    channel.consume(
      ...
      async (msg) => {
        if (msg) {
          ...
          try {
            await gerarCertificadoPdfService(emailData);
            channel.ack(msg); 
          } catch (error) {
